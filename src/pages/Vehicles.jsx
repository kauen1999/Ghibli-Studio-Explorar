...
const Vehicles = () => {
  ...
  useEffect(()=>{
    const fetchVehicles = async () => {
      ...
      setVehicles(response.vehiclesDetails);
    }
    fetchVehicles();
  }, [])
  return (
    <div className="text-white ">
      ...
      <div className="p-[20px] grid grid-cols-4 gap-4 justify-items-center">
        {vehicles.map((vehicle)=>(
          <div key={vehicle.id} className="card w-70 h-auto bg-[#07182E] rounded-2xl">
          <div className="relative z-10 p-4 card-content">
            ...
            <div className="mb-4">
              ...
              <div className="flex flex-wrap -mx-1">
                ...
                <div className="px-2 py-1 m-0.5 bg-white/10 rounded-full text-xs font-medium text-white/70 shadow-sm border border-white/10">
                {vehicle.vehicle_class} Eyes
                </div>
                <div className="px-2 py-1 m-0.5 bg-white/10 rounded-full text-xs font-medium text-white/70 shadow-sm border border-white/10">
                {vehicle.length} Hair
                </div>
                <div className="px-2 py-1 m-0.5 bg-white/10 rounded-full text-xs font-medium text-white/70 shadow-sm border border-white/10">
                {vehicle.pilot.name} Hair
                </div>
              </div>
            </div>
            <div className="mb-4">
